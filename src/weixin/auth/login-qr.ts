...
import { listLocalBotTokens } from "./token.js";
...
export async function startWeixinLoginWithQr(opts: {
  ...
  try {
    ...
    const qrResponse = await fetchQRCode(FIXED_BASE_URL, botType, opts.storageDir);
    logger.info(
...
async function refreshQRCode(
  ...
  try {
    const qrResponse = await fetchQRCode(FIXED_BASE_URL, botType, storageDir);
